...
async function loadExisting(): Promise<Record<string, PhotoRow>> {
  try {
    const mod = await import(path.join(ROOT, 'src', 'data', 'photos.ts'));
    return (mod.shopPhotos as Record<string, PhotoRow>) ?? {};
  } catch {
    return {};
  }
}
...
async function main() {
  ...
  const existing = skipExisting ? await loadExisting() : {};
  const result: Record<string, PhotoRow> = { ...existing };
  let i = 0;
  let processed = 0;
  for (const shop of shops) {
    i++;
    if (skipExisting && existing[shop.id] && existing[shop.id].image_paths.length > 0) {
      console.log(`[${i}/${shops.length}] ${shop.name} -> skip (already has ${existing[shop.id].image_paths.length} photos)`);
      continue;
    }
  ...
  console.log(`\nProcessed ${processed} shops this run.`);
...
`;
  ...
  console.log(`Done. Total: ${hits}/${shops.length} shops with metadata, ${totalImages} photos.`);
}
